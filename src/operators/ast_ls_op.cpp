...
void LsOp::EmitRISC(std::ostream &stream, Context &context, int destReg) const {
    ...
    stream << "sll x" << destReg << ", x" << left_reg << ", x"<< right_reg << std::endl;

    context.freeRegister(left_reg);
    context.freeRegister(right_reg);
}

void LsOp::Print(std::ostream &stream) const {
    left_->Print(stream);
    stream << " << ";
    right_->Print(stream);
}
void LsOp::isFunction(Context &context) const{
